...
class VRPBEnv:
    ...
    def load_problems(self, batch_size, problems=None, aug_factor=1):
        ...
        # reset pomo_size based on the *original* problem size before padding might occur
        # This assumes self.problem_size reflects the intended original size, not the padded one.
        # If subproblems can have varying sizes, this needs careful adjustment.
        # Let's assume self.problem_size is the maximum possible size *before* padding.
        # For VRPB, pomo size should be based on linehaul nodes count.
        # We'll calculate it dynamically after filtering.

        # Original START_NODE calculation (problematic)
        ...
        # self.START_NODE = self.START_NODE[node_demand > 0].reshape(self.batch_size, -1)[:, :self.pomo_size] # <-- Error Source

        # --- New START_NODE Calculation ---
        start_node_candidates_list = []
        max_linehaul_count = 0
        all_node_indices = torch.arange(start=1, end=node_demand.size(1) + 1, device=self.device) # Indices up to padded size

        for i in range(self.batch_size):
            instance_demand = node_demand[i] # Demand for instance i (potentially padded)
            instance_indices = all_node_indices # All possible node indices for this instance size

            # Filter linehaul nodes (demand > 0) for this instance
            linehaul_mask = instance_demand > 0
            linehaul_nodes = instance_indices[linehaul_mask]

            # Pad or truncate linehaul nodes for this instance if needed later, store for now
            start_node_candidates_list.append(linehaul_nodes)
            max_linehaul_count = max(max_linehaul_count, len(linehaul_nodes))

        # Determine the effective pomo_size based on the available linehaul nodes across the batch
        # Use the minimum count across the batch if strict, or pad/truncate?
        # Let's use the minimum of original pomo_size and max_linehaul_count found
        # Ensure pomo_size is at least 1 if possible
        effective_pomo_size = min(self.pomo_size, max_linehaul_count) if max_linehaul_count > 0 else 1
        # Update self.pomo_size for consistency in the rest of the environment
        self.pomo_size = effective_pomo_size

        # Pad/truncate each instance's candidates and stack
        processed_start_nodes = []
        for candidates in start_node_candidates_list:
            if len(candidates) >= effective_pomo_size:
                processed_start_nodes.append(candidates[:effective_pomo_size])
            else:
                # Pad with a value unlikely to be chosen (e.g., 0 or -1) if fewer candidates than pomo_size
                # Using 0 (depot) might be problematic if depot selection is valid later
                # Let's use the last valid candidate if padding is needed, or 0 if no candidates exist
                padding_val = candidates[-1] if len(candidates) > 0 else torch.tensor(0, device=self.device) # Use last candidate or 0 if empty
                num_to_pad = effective_pomo_size - len(candidates)
                if num_to_pad < 0: num_to_pad = 0 # Should not happen based on effective_pomo_size logic

                padding_tensor = padding_val.repeat(num_to_pad)
                padded_candidates = torch.cat([candidates, padding_tensor])
                processed_start_nodes.append(padded_candidates[:effective_pomo_size]) # Ensure correct size

        self.START_NODE = torch.stack(processed_start_nodes, dim=0)
        # --- End New START_NODE Calculation ---
